...
#pragma region pools and buffers
...
#pragma endregion
#pragma region fences and semaphores
...
VkSemaphoreSubmitInfo vkinit::semaphore_submit_info(VkPipelineStageFlags2 stageMask, VkSemaphore semaphore) 
{
	...
	submitInfo.deviceIndex = 0; // we can also utilize it to run on several gpus
	submitInfo.value = 1; // we can also make counter type semaphores 
	return submitInfo;
}
...
VkImageCreateInfo vkinit::image_create_info(VkFormat format, VkImageUsageFlags usageFlags, VkExtent3D extent)
{
    ...
    info.mipLevels = 1;
    info.arrayLayers = 1;
    info.samples = VK_SAMPLE_COUNT_1_BIT; // no msaa by default (1 sample per pixel)
